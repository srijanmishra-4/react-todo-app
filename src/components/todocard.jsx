const TodoCard = ({ uid, tid, title, date, status, onTaskUpdate, onEdit }) => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-md p-4 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl relative mb-6 sm:mb-8 md:mb-10">
            <div className="flex items-center text-gray-500 text-sm sm:text-base md:text-lg mb-4">
                <i className="far fa-calendar-alt mr-2"></i>
                <span>{date}</span>
            </div>
            <div className="text-lg sm:text-xl md:text-2xl font-bold flex items-center mb-6 break-words">
                <i className="fas fa-arrow-right mr-2"></i>
                <span className="break-words overflow-hidden w-full">{title}</span>
            </div>
